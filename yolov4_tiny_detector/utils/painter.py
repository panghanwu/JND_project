...
def mark_bboxes(image, bboxes, colors, show_text=True, show_cfd=True, line_thick=3, text_size=1):
    img = image.copy()
    if show_text:
...
def mark_centers(image, bboxes, colors, show_text=True, point_size=6, text_size=1):
    img = image.copy()
    if show_text:
        text_size = np.floor(text_size * 3e-2 * np.shape(img)[1] + 0.5).astype('int32')
        font = ImageFont.truetype(font='simhei.ttf', size=text_size)
    ...
    for bbox in sorted(bboxes, key=lambda s: s[2]):
        ...
        draw.ellipse(point_loc, fill=colors[bbox[0]], outline=None, width=1)
        # tag
        if show_text:
            tag = '{}'.format(bbox[1])
            text_loc = [x, y-2*point_size]
            draw.text(text_loc, tag, anchor='ms', fill=colors[bbox[0]], font=font)
    del draw
    return img